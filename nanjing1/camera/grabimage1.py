...
def identify_different_devices(deviceList):
    ...
    for i in range(0, deviceList.nDeviceNum):
        ...
        if mvcc_dev_info.nTLayerType == MV_GIGE_DEVICE:
            ...
            device_info[stSerialNumber.rstrip('\x00')]=i
            # 获取用户自定义名称
            stUserDefinedName = ""
...
def creat_camera(deviceList , nConnectionNum ,log = True , log_path = getcwd()):
    ...
    # 选择设备并创建句柄
    stDeviceList = cast(deviceList.pDeviceInfo[int(nConnectionNum)], POINTER(MV_CC_DEVICE_INFO)).contents # type: ignore
    if log == True:
...
def create_byIP(stDevInfo):
    cam=MvCamera()
    ret = cam.MV_CC_CreateHandle(stDevInfo)
    if ret != 0:
        # print ("open device fail! ret[0x%x]" % ret)
        print("open camera by ip failed!")
    return cam
...
def access_get_image(cam , active_way = "getImagebuffer"):
    ...
    if active_way == "getImagebuffer":
        ...
        stParam = MVCC_INTVALUE_EX() # type: ignore
        memset(byref(stParam), 0, sizeof(MVCC_INTVALUE_EX)) # type: ignore
        ...
        stFrameInfo = MV_FRAME_OUT_INFO_EX() # type: ignore
        ...
        if ret == 0:
            ...
            print("no data[0x%x]" % ret)
        MvCamera.MV_CC_ClearImageBuffer()

 # 按键图像采集
def Press_get_image(cam , active_way = "getImagebuffer",output_path="mypic.jpg"):
    """
    :param cam:     相机实例
    :active_way:主动取流方式的不同方法 分别是（getImagebuffer）（getoneframetimeout）
    :return:
    """
    if active_way == "getImagebuffer":
        stOutFrame = MV_FRAME_OUT() # type: ignore
        memset(byref(stOutFrame), 0, sizeof(stOutFrame))
        while True:
            Key_value = input("按键3拍照：")
            while int(Key_value) == 3:
                ret = cam.MV_CC_GetImageBuffer(stOutFrame, 1000)
                if None != stOutFrame.pBufAddr and 0 == ret and stOutFrame.stFrameInfo.enPixelType == 17301505:
                    print("get one frame: Width[%d], Height[%d], nFrameNum[%d]" % (stOutFrame.stFrameInfo.nWidth, stOutFrame.stFrameInfo.nHeight, stOutFrame.stFrameInfo.nFrameNum))
                    pData = (c_ubyte * stOutFrame.stFrameInfo.nWidth * stOutFrame.stFrameInfo.nHeight)()
                    cdll.msvcrt.memcpy(byref(pData), stOutFrame.pBufAddr,stOutFrame.stFrameInfo.nWidth * stOutFrame.stFrameInfo.nHeight)
                    data = np.frombuffer(pData, count=int(stOutFrame.stFrameInfo.nWidth * stOutFrame.stFrameInfo.nHeight),dtype=np.uint8)
                    image_control2(data=data, stFrameInfo=stOutFrame.stFrameInfo,output_path=output_path)
                elif None != stOutFrame.pBufAddr and 0 == ret and stOutFrame.stFrameInfo.enPixelType == 17301514:
                    print("get one frame: Width[%d], Height[%d], nFrameNum[%d]" % (stOutFrame.stFrameInfo.nWidth, stOutFrame.stFrameInfo.nHeight, stOutFrame.stFrameInfo.nFrameNum))
                    pData = (c_ubyte * stOutFrame.stFrameInfo.nWidth * stOutFrame.stFrameInfo.nHeight)()
                    cdll.msvcrt.memcpy(byref(pData), stOutFrame.pBufAddr,stOutFrame.stFrameInfo.nWidth * stOutFrame.stFrameInfo.nHeight)
                    ...
                    data = np.frombuffer(pData, count=int(stOutFrame.stFrameInfo.nWidth * stOutFrame.stFrameInfo.nHeight * 2),dtype=np.uint8)
                    image_control2(data=data, stFrameInfo=stOutFrame.stFrameInfo,output_path=output_path)
                elif None != stOutFrame.pBufAddr and 0 == ret and stOutFrame.stFrameInfo.enPixelType == 17301513:
                    print("get one frame: Width[%d], Height[%d], nFrameNum[%d]" % (stOutFrame.stFrameInfo.nWidth, stOutFrame.stFrameInfo.nHeight, stOutFrame.stFrameInfo.nFrameNum))
                    pData = (c_ubyte * stOutFrame.stFrameInfo.nWidth * stOutFrame.stFrameInfo.nHeight)()
                    cdll.msvcrt.memcpy(byref(pData), stOutFrame.pBufAddr,stOutFrame.stFrameInfo.nWidth * stOutFrame.stFrameInfo.nHeight)
                    data = np.frombuffer(pData, count=int(stOutFrame.stFrameInfo.nWidth * stOutFrame.stFrameInfo.nHeight),dtype=np.uint8)
...
def one_shot_image(cam , active_way = "getImagebuffer",output_path="mypic.jpg"):
    """
    :param cam:     相机实例
    :active_way:主动取流方式的不同方法 分别是（getImagebuffer）（getoneframetimeout）
    :return:
    ...
    if active_way == "getImagebuffer":
        ...
        if ret == 0:
            print("get one frame: Width[%d], Height[%d], nFrameNum[%d] " % (stFrameInfo.nWidth, stFrameInfo.nHeight, stFrameInfo.nFrameNum))
            image = np.asarray(pData)
            image_control2(data=image, stFrameInfo=stFrameInfo,output_path=output_path)
        else:
            print("no data[0x%x]" % ret)
...
stFrameInfo = POINTER(MV_FRAME_OUT_INFO_EX) # type: ignore
pData = POINTER(c_ubyte)
FrameInfoCallBack = winfun_ctype(None, pData, stFrameInfo, c_void_p)
def image_callback(pData, pFrameInfo, pUser):
    global img_buff
    img_buff = None
    stFrameInfo = cast(pFrameInfo, POINTER(MV_FRAME_OUT_INFO_EX)).contents # type: ignore
    if stFrameInfo:
        print ("get one!!!! frame: Width[%d], Height[%d], nFrameNum[%d]" % (stFrameInfo.nWidth, stFrameInfo.nHeight, stFrameInfo.nFrameNum))
...
# 事件回调
stEventInfo = POINTER(MV_EVENT_OUT_INFO) # type: ignore
pData = POINTER(c_ubyte)
EventInfoCallBack = winfun_ctype(None, stEventInfo, c_void_p)
def event_callback(pEventInfo, pUser):
    stPEventInfo = cast(pEventInfo, POINTER(MV_EVENT_OUT_INFO)).contents # type: ignore
...
def main():
    ...
    # 主动取流方式抓取图像
    #access_get_image(cam, active_way="getImagebuffer")
    Press_get_image(cam, active_way="getImagebuffer")
    # 关闭设备与销毁句柄
    close_and_destroy_device(cam)
